...
async function getAllEvents() {
  try {
    ...
    querySnapshot.forEach((doc) => {
      ...
      const card = `
        <div class="bg-white shadow-md rounded-lg overflow-hidden card-bg">
...
async function likeEvent(e) {
  if (auth.currentUser) {
    e.disabled = true;
    const docRef = doc(db, "events", e.id);
    if (e.innerText.includes("Liked..")) {
      updateDoc(docRef, {
        likes: arrayRemove(auth.currentUser.uid),
      })
        .then(() => {
          e.innerText = `Like ${parseInt(e.innerText.split(" ")[1]) - 1}`;
          e.disabled = false;
        })
        .catch((err) => console.log(err));
    } else {
      updateDoc(docRef, {
        ...
        .then(() => {
          e.innerText = `Liked.. ${parseInt(e.innerText.split(" ")[1]) + 1}`;
          e.disabled = false;
        })
        .catch((err) => console.log(err));
    }
